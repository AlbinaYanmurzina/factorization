"""
backend/algorithms/quadratic_sieve_lpv.py

Квадратичное решето с вариацией множителя (раздел 6.8 учебного пособия).

LPV (Large Prime Variation) - важная оптимизация квадратичного решета,
предложенная Pomerance и Silverman в 1987 году.

Проблема базового QS:
Отбрасываются числа Q(x), которые "почти гладкие" - раскладываются
на простые из FB, но имеют один большой простой делитель P > B.

Идея LPV:
Принимаем "полугладкие" числа вида Q(x) = smooth_part × P,
где B < P < B² (один большой простой).

Ключевое наблюдение:
Если найдены два полугладких числа с ОДИНАКОВЫМ большим простым P:
  Q(x₁) = S₁ × P
  Q(x₂) = S₂ × P

То их произведение Q(x₁) · Q(x₂) = S₁ · S₂ · P² полностью гладкое!
(P² уходит под квадратный корень)

Алгоритм:
1. Просеивание с расширенным допуском (принимаем кандидатов с остатком < B²)
2. Пробное деление: если остаток = 1 → гладкое, если 1 < остаток < B² → полугладкое
3. Полугладкие сохраняем в словарь partials[P] = {x, exponents}
4. При повторном P: склеиваем пару в одно соотношение
5. Комбинированное соотношение: X = x₁·x₂, exponents = sum(exponents)

Преимущества:
- Увеличивает количество используемых соотношений на 30-50%
- Не требует увеличения факторной базы
- Минимальные дополнительные вычисления
- Легко комбинируется с другими оптимизациями

Сложность: та же L_n[1/2, 1], но с меньшей константой

Применимость:
Стандартная оптимизация, используется во всех современных реализациях QS.
Особенно эффективна для чисел 60-80 бит.

Историческое значение:
LPV сделал квадратичное решето конкурентоспособным с эллиптическими кривыми
для чисел средней величины и стал основой для Double Large Prime Variation.
"""
...
class QuadraticSieveLPV(FactorizationAlgorithm):
    """
    Квадратичное решето с вариацией множителя (Large Prime Variation).
    
    Раздел 6.8 учебного пособия: "Вариация множителя в методе квадратичного решета"
    
    Принимает не только B-гладкие числа, но и полугладкие числа вида
    Q(x) = smooth_part × P, где B < P < B² (один большой простой).
    Два полугладких с одинаковым P склеиваются в одно гладкое соотношение.
    
    Attributes:
        steps_log: Лог шагов работы алгоритма (наследуется от базового класса)
    """
    def __init__(self):
        """Инициализация LPV версии квадратичного решета."""
        super().__init__()
