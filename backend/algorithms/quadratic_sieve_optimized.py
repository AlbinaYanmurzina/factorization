"""
backend/algorithms/quadratic_sieve_optimized.py

Оптимизированная реализация метода квадратичного решета (раздел 6.2 учебного пособия).

Реализует метод Померанца с двумя ключевыми оптимизациями:
1. Алгоритм Тонелли-Шенкса для предвычисления корней
2. Логарифмическое просеивание вместо пробного деления

Метод Померанца (1981):
Усовершенствование алгоритма Диксона, использующее просеивание для
эффективного поиска B-гладких чисел.

Ключевые улучшения:

1. Предвычисление корней (Тонелли-Шенкс):
   Для каждого p из FB находим корни r₁, r₂: r² ≡ n (mod p)
   Это означает, что Q(x) = x² - n ≡ 0 (mod p) при x ≡ r₁ или r₂ (mod p)

2. Логарифмическое просеивание:
   Вместо пробного деления используем массив sieve[M]:
   - Для каждого p добавляем log₂(p) в позиции i ≡ r₁, r₂ (mod p)
   - После просеивания: sieve[i] ≈ log₂(Q(xᵢ)) для гладких чисел
   - Кандидаты: позиции где sieve[i] ≥ log₂(Q(xᵢ)) - tolerance

Сложность:
- Пробное деление (Basic): O(M · |FB|)
- Просеивание (Optimized): O(M · log log B)
- Ускорение: в 50-100 раз!

Память: O(M) для массива просеивания

Применимость:
- Эффективен для чисел 50-70 бит
- Основа для всех современных вариантов QS
- Используется в криптоанализе RSA

Историческое значение:
Метод Померанца сделал квадратичное решето практически применимым,
превратив его из теоретического алгоритма в рабочий инструмент.
"""
...
class QuadraticSieveOptimized(FactorizationAlgorithm):
    """
    Оптимизированная реализация метода квадратичного решета (метод Померанца).
    
    Использует логарифмическое просеивание и алгоритм Тонелли-Шенкса
    для эффективного поиска B-гладких чисел.
    ...
    def __init__(self):
        """Инициализация оптимизированного квадратичного решета."""
        super().__init__()

    def _get_factor_base(self, n: int, B: int) -> List[Dict]:
        """
        Построение факторной базы с предвычислением корней.
        
        Для каждого простого p из FB вычисляем корни r₁, r₂ уравнения
        x² ≡ n (mod p) через алгоритм Тонелли-Шенкса.
        
        Эти корни используются для инициализации просеивания:
        позиции i, где x_start + i ≡ r₁ или r₂ (mod p), делятся на p.
        ...
        Returns:
            List[Dict]: Факторная база, каждый элемент содержит:
                       - 'p': простое число
                       - 'r1', 'r2': корни x² ≡ n (mod p)
                       - 'log_p': log₂(p) для просеивания
        
        Примечание:
            Для p = 2 корни тривиальны: r₁ = r₂ = n mod 2
        """
    ...
    def _sieve_and_find_smooth(self, n: int, factor_base: List[Dict], required_count: int) -> List[Dict]:
        """
        Поиск B-гладких чисел методом логарифмического просеивания.
        
        Метод Померанца (раздел 6.2): вместо пробного деления используем
        массив sieve[M], где накапливаем логарифмы делителей.
        
        Алгоритм:
        1. Инициализация: sieve[i] = 0 для всех i ∈ [0, M)
        2. Для каждого p из FB:
           - Находим стартовые позиции: i₁ ≡ r₁ - x_start (mod p)
                                        i₂ ≡ r₂ - x_start (mod p)
           - Добавляем log₂(p) в позиции i₁, i₁+p, i₁+2p, ...
           - Добавляем log₂(p) в позиции i₂, i₂+p, i₂+2p, ...
        3. После просеивания: sieve[i] ≈ log₂(Q(x_start + i))
        4. Кандидаты: позиции где sieve[i] ≥ log₂(Q(xᵢ)) - tolerance
        5. Проверка кандидатов пробным делением
        
        Преимущества:
        - Сложность: O(M · log log B) вместо O(M · |FB|)
        - Ускорение: в 50-100 раз по сравнению с пробным делением
        - Параллелизуемость: можно просеивать разные интервалы независимо
        ...
        Returns:
            List[Dict]: Список гладких чисел с разложениями
        
        Raises:
            ValueError: Если найдено недостаточно гладких чисел
        
        Примечание:
            Tolerance (допуск) компенсирует ошибки округления логарифмов.
            Обычно tolerance ≈ log₂(p_max), где p_max - наибольшее простое в FB.
        """
        M = 50000
